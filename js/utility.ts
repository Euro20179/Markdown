...
function saveFile(){
    const downloadB = <HTMLAnchorElement>document.getElementById("download")
    let file = new Blob([preview.innerHTML], {type:"text/markdown"})
    downloadB.href = URL.createObjectURL(file)
    const fileName = (<HTMLInputElement>document.getElementById("file-name")).value
...
function savePlain(){
    const downloadB = <HTMLAnchorElement>document.getElementById("download-plain")
...
function printMe(elem){
    var myframe = <HTMLIFrameElement>document.createElement('IFRAME');
...
function addTextTypeInTextArea(text, selectType: SelectionMode="end"){
    actionHistory.add()
    let el = textEditor;
...
function mulString(str: string, amount: number){
